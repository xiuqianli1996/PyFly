...
def upload(name=None):
    if request.method == 'POST':
        if not current_user.is_authenticated:
            return jsonify(models.BaseResult(code=1, msg='请先登录'))
        file = request.files['smfile']
        if not file:
            return jsonify(models.BaseResult(code=1, msg='没有上传任何文件'))
        try:
            filename = upload_photos.save(file)
        except UploadNotAllowed:
            return jsonify(models.BaseResult(code=1, msg='不支持的文件格式'))
         #因为存储在帖子内容里，所以用个相对路径，方便数据转移
        file_url = '/api/upload/' + filename
        result = models.BaseResult(data={'url': file_url}).put('code', 0)
...
def post_adopt(comment_id):
    ...
    if post['user_id'] != current_user.user['_id']:
        jsonify(models.BaseResult(msg='没有权限'))
    if post.get('accepted', False):
        jsonify(models.BaseResult(msg='已有被采纳回答'))
...
def post_reply():
    ...
    if not content:
        return jsonify(models.BaseResult(1, '内容不能为空'))
    ...
    return jsonify(models.R().ok().put('msg', '回帖成功'))
...
def reply_delete(comment_id):
    ...
    return jsonify(models.R().ok().put('msg', '删除成功'))
...
def get_reply_content(comment_id):
    ...
    return jsonify(models.BaseResult(data=comment['content']))
